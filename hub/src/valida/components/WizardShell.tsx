...
function dz(val: string, err: boolean, dark: boolean) {
  return {
    bg:        val ? (dark ? '#0F2420' : '#F2FAF9') : (dark ? '#1F1F23' : '#FBFCFC'),
    border:    val ? (dark ? '#1F5C4D' : '#9ED2CB') : (err ? (dark ? '#7A362E' : '#E9B5AF') : (dark ? '#3F3F46' : '#D6DBDF')),
    iconColor: val ? '#0F8B81' : (dark ? '#71717A' : '#A7B1B8'),
    text:      val || 'Adjuntar un archivo',
    textColor: val ? (dark ? '#4ADE94' : '#0C6E66') : (dark ? '#A1A1AA' : '#6B7780'),
  };
}

function AttachButton({ val, err, icon, label, onClick, dark }: {
  val: string; err: boolean; icon: React.ReactNode; label: string; onClick: () => void; dark: boolean;
}) {
  const d = dz(val, err, dark);
  return (
    <button
      onClick={onClick}
    ...
    >
      <div className="w-9 h-9 rounded-[9px] bg-white dark:bg-neutral-900 border border-[#E4E7EB] dark:border-neutral-700 flex items-center justify-center flex-none">
        {icon}
      </div>
      <div className="flex-1">
        <div className="text-[13.5px] font-semibold" style={{ color: d.textColor }}>{d.text}</div>
        <div className="text-[11.5px] text-[#8A969E] dark:text-neutral-500 mt-0.5">{label} · obligatorio</div>
      </div>
    </button>
  );
}
...
export default function WizardShell({
  ...
  const req = (v: string) => showErrors && !v.trim() ? '#D9483B' : (isDarkMode ? '#3F3F46' : '#D6DBDF');
  ...
  const proto   = dz(form.protocolo, showErrors && !form.protocolo, isDarkMode);
  const hojasDz = dz(form.hojas,     showErrors && !form.hojas, isDarkMode);
  const bitaDz  = dz(form.bitacoras, showErrors && !form.bitacoras, isDarkMode);
  ...
  return (
    <main className="max-w-[880px] mx-auto px-7 pt-7 pb-16">
      ...
      <button
        onClick={onCancel}
        className="inline-flex items-center gap-[7px] py-1.5 bg-transparent border-none font-medium text-[13px] text-[#8A969E] dark:text-neutral-500 cursor-pointer hover:text-[#44525A] dark:hover:text-neutral-300 transition-colors"
      >
        <ArrowLeftIcon className="w-[15px] h-[15px]" />
        Volver a informes
      </button>
      <h1 className="mt-2.5 mb-0 text-[24px] font-bold tracking-[-0.01em] text-[#1A2B32] dark:text-neutral-100">{title}</h1>
          ...
          return (
            <div key={s.num} className="flex items-center gap-[11px]" style={{ flex: isLast ? undefined : 1 }}>
              <div
                ...
                style={{
                  background: done ? '#0F8B81' : (active ? (isDarkMode ? '#123326' : '#E6F4F2') : (isDarkMode ? '#18181B' : '#fff')),
                  color: done ? '#fff' : (active ? (isDarkMode ? '#4ADE94' : '#0C6E66') : (isDarkMode ? '#71717A' : '#A7B1B8')),
                  borderColor: (done || active) ? '#0F8B81' : (isDarkMode ? '#3F3F46' : '#D6DBDF'),
                }}
              >
                {done
              ...
              <div className="text-[13px] font-semibold whitespace-nowrap" style={{ color: active ? (isDarkMode ? '#F4F4F5' : '#26343B') : (done ? (isDarkMode ? '#D4D4D8' : '#44525A') : (isDarkMode ? '#71717A' : '#A7B1B8')) }}>
                {s.label}
              </div>
              {!isLast && <div className="h-[1.5px] flex-1 mx-1" style={{ background: done ? '#0F8B81' : (isDarkMode ? '#3F3F46' : '#E4E7EB') }} />}
            </div>
          );
        })}
      </div>

      <div className="bg-white dark:bg-neutral-900 border border-[#E4E7EB] dark:border-neutral-800 rounded-[16px] px-7 py-[26px]">

        {/* PASO 1 */}
        {step === 1 && (
          <div>
            <div className="text-[16px] font-bold text-[#1A2B32] dark:text-neutral-100">Datos del informe</div>
            <div className="text-[13px] text-[#8A969E] dark:text-neutral-500 mt-[3px]">
            ...
            <div className="grid grid-cols-2 gap-x-[18px] gap-y-4 mt-[22px]">
              {([
                ...
                <div key={f.key}>
                  <label className="block text-[12.5px] font-semibold text-[#44525A] dark:text-neutral-300 mb-1.5">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-[10px] text-[14px] text-[#1A2B32] dark:text-neutral-100 border rounded-[9px] bg-white dark:bg-neutral-900 focus:border-[#0F8B81]"
              ...
              <div className="col-span-2">
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full px-3 py-[10px] text-[14px] text-[#1A2B32] dark:text-neutral-100 border rounded-[9px] bg-white dark:bg-neutral-900 resize-y focus:border-[#0F8B81]"
        ...
        {step === 2 && (
          <div>
            <div className="text-[16px] font-bold text-[#1A2B32] dark:text-neutral-100">Métodos analíticos y adjuntos</div>
            <div className="text-[13px] text-[#8A969E] dark:text-neutral-500 mt-[3px]">Selecciona los grupos de documentos a validar.</div>
              ...
              {METHODS.map(name => {
                ...
                return (
                  <button
                    ...
                    style={{
                      background: on ? (isDarkMode ? '#0F2420' : '#F2FAF9') : (isDarkMode ? '#18181B' : '#fff'),
                      borderColor: on ? (isDarkMode ? '#1F5C4D' : '#9ED2CB') : (isDarkMode ? '#3F3F46' : '#E4E7EB'),
                    }}
                  >
                    <span
                      className="w-[19px] h-[19px] rounded-[6px] flex items-center justify-center flex-none border-[1.5px]"
                      style={{ background: on ? '#0F8B81' : (isDarkMode ? '#18181B' : '#fff'), borderColor: on ? '#0F8B81' : (isDarkMode ? '#52525B' : '#C9D0D5') }}
                    >
                      {on && <CheckIcon className="w-[11px] h-[11px] text-white" strokeWidth={3} />}
                    </span>
                    <span className="text-[13px] font-semibold" style={{ color: on ? (isDarkMode ? '#4ADE94' : '#0C6E66') : (isDarkMode ? '#D4D4D8' : '#44525A') }}>{name}</span>
                  </button>
                );
              })}
            </div>
            {selectedGroupNames.length > 0 ? (
              <div className="mt-6 pt-5 border-t border-[#EEF0F2] dark:border-neutral-800">
                <div className="text-[13px] font-bold tracking-[.04em] uppercase text-[#8A969E] dark:text-neutral-500">Adjuntos requeridos</div>
                <div className="flex flex-col gap-3 mt-3.5">
                  {selectedGroupNames.map(name => {
                    const g = groups[name] || { selected: true, doc: 'Reporte LIMS', file: '' };
                    const z = dz(g.file || '', false, isDarkMode);
                    return (
                      <div key={name} className="grid gap-3 items-end p-[14px] border border-[#EEF0F2] dark:border-neutral-800 rounded-[11px] bg-[#FBFCFC] dark:bg-neutral-800/40" style={{ gridTemplateColumns: '1fr 200px' }}>
                        <div className="col-span-2 flex items-center gap-2 text-[13.5px] font-semibold text-[#26343B] dark:text-neutral-100">
                          <span className="w-[7px] h-[7px] rounded-full bg-[#0F8B81]" />{name}
                        </div>
                        <div>
                          <label className="block text-[11.5px] font-semibold text-[#8A969E] dark:text-neutral-500 mb-[5px]">Archivo de soporte</label>
                          <button
                        ...
                        <div>
                          <label className="block text-[11.5px] font-semibold text-[#8A969E] dark:text-neutral-500 mb-[5px]">Documento <span className="text-[#D9483B]">*</span></label>
                          <select
                            value={g.doc || 'Reporte LIMS'}
                            onChange={e => setGroupDoc(name, e.target.value)}
                            className="w-full px-[11px] py-[9px] font-medium text-[13px] text-[#26343B] dark:text-neutral-100 border border-[#D6DBDF] dark:border-neutral-700 rounded-[9px] bg-white dark:bg-neutral-900 cursor-pointer"
                          >
              ...
              <div className="mt-[18px] p-[18px] text-center bg-[#FBFCFC] dark:bg-neutral-800/40 border border-dashed border-[#D6DBDF] dark:border-neutral-700 rounded-[11px] text-[12.5px] text-[#8A969E] dark:text-neutral-500">
        ...
        {step === 3 && (
          <div>
            <div className="text-[16px] font-bold text-[#1A2B32] dark:text-neutral-100">Soportes de preparación</div>
            <div className="text-[13px] text-[#8A969E] dark:text-neutral-500 mt-[3px]">Adjunta los documentos de preparación que respaldan el proceso.</div>
            ...
            <div className="mt-5 flex items-start gap-2.5 px-[15px] py-[13px] bg-[#F8FBFB] dark:bg-neutral-800/50 border border-[#DDEAE8] dark:border-[#1F5C4D] rounded-[11px]">
              <InformationCircleIcon className="w-4 h-4 text-[#0F8B81] dark:text-[#2DD4C4] flex-none mt-px" />
              <div className="text-[12.5px] text-[#3F8F86] dark:text-[#6FBFB2] leading-relaxed">
        ...
        {/* Navegación */}
        <div className="flex items-center justify-between mt-[26px] pt-5 border-t border-[#EEF0F2] dark:border-neutral-800">
          <button
            onClick={back}
            className="inline-flex items-center gap-[7px] px-4 py-[10px] bg-white dark:bg-neutral-900 border border-[#D6DBDF] dark:border-neutral-700 rounded-[9px] font-semibold text-[13.5px] text-[#44525A] dark:text-neutral-300 cursor-pointer hover:bg-[#F2F4F6] dark:hover:bg-neutral-800 transition-colors"
          >
